    ...
    for mdc_f in tqdm(mdcath_files, total=len(mdcath_files)):
        ...
        for temp in TEMPS:
            for rep in REPS:
                ...
                update_h5_dataset(
                    f"{pdb_code}/T{temp}/R{rep}/xyz", torch.from_numpy(traj.xyz)
                )
                update_h5_dataset(
                    f"{pdb_code}/T{temp}/R{rep}/time", torch.from_numpy(traj.time)
                )
                update_h5_dataset(
                    f"{pdb_code}/T{temp}/R{rep}/rmsf",
                ...
                update_h5_dataset(
                    f"{pdb_code}/T{temp}/R{rep}/gyration",
                    md.compute_gyration_tensor(traj),
                )
                # update_h5_dataset(f"{pdb_code}/T{temp}/R{rep}/principal_moments", md.principal_moments(traj)
                update_h5_dataset(
                    f"{pdb_code}/T{temp}/R{rep}/ca_distances",
                    md.geometry.squareform(
                        *md.compute_contacts(
                            traj[0], scheme="ca", ignore_nonprotein=True
                        )
                    ),
                )
