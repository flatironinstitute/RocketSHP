...
def analyze_and_generate_config(sweep_id: str, output_dir: str = "configs/optimized"):
    ...
    project = neptune.init_project(
        project="samsl-flatiron/PLANET-MD", api_token=os.getenv("NEPTUNE_API_TOKEN")
    )
    ...
    optimized_config = {
        ...
        if pd.notna(best_run["batch_size"])
        else 16,
        ...
        if pd.notna(best_run["rmsf_alpha"])
        else 1.0,
        "ca_dist_alpha": float(best_run["ca_dist_alpha"])
        if pd.notna(best_run["ca_dist_alpha"])
        else 1.0,
        "gcc_lmi_alpha": float(best_run["gcc_lmi_alpha"])
        if pd.notna(best_run["gcc_lmi_alpha"])
        else 1.0,
        "shp_alpha": float(best_run["shp_alpha"])
        if pd.notna(best_run["shp_alpha"])
        else 0.01,
        # Fixed parameters
        "embedding_dim": 1536,
        "output_dim": 1,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Output directory for optimized config",
    )
