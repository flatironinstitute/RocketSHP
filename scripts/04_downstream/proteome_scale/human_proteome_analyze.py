...
def parse_uniprot_xml_biotite(up_results):
        ...
        try:
            ...
            for comment in comment_elems:
                ...
                if comment_type == "subcellular location":
                    ...
                    for loc in loc_elems:
                        if (
                            loc.text
                            and loc.text not in protein_info["subcellular_location"]
                        ):
                ...
                if feature_type == "region of interest":
                    ...
                    if location is not None:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Invalid position for {uniprot_id}: {begin}, {end}"
                            )
            ...
            for pfam in pfam_elems:
                ...
                if pfam_id:
                    ...
                    protein_info["domains"] = list(
                        set(protein_info["domains"])
                    )  # Remove duplicates
    ...
    result_df["gene_names"] = result_df["gene_names"].apply(
...
with open(result_dir / "rocketshp_results.csv", "r") as f:
    for line in tqdm(f):
        ...
        if len(rmsf) == 1400:
            continue
        planet_md_rmsf[pid] = rmsf
# %% Get annotations for a sub-sample

subsample_size = len(planet_md_rmsf)
random_state = 42
np.random.seed(random_state)
subsample_prots = np.random.choice(
    list(planet_md_rmsf.keys()), size=subsample_size, replace=False
)
...
def boxplot_regions(region_type, search_query):
    ...
    for k in tqdm(subsample_prots):
        pred_rmsf = planet_md_rmsf[k]
        if len(pred_rmsf) == 1400:
            continue

        # get regions from annotations_df
        regions = annotations_df.loc[
            annotations_df["uniprot_id"] == k, "regions"
        ].values
        try:
            regions = regions[0]
        except:
    ...
    type_melt = type_df.melt(
        id_vars="uniprot_id", value_vars=["type_rmsf", "other_rmsf"]
    )
...
for k in tqdm(subsample_prots):
    # get regions from annotations_df
    subcellular_location = annotations_df.loc[
        annotations_df["uniprot_id"] == k, "subcellular_location"
    ].values
    try:
...
subcellular_location_counts = subcellular_location_counts.sort_values(
...
subcellular_location_counts = (
    subcellular_location_counts.groupby("subcellular_location").sum().reset_index()
)
subcellular_location_counts = subcellular_location_counts.sort_values(
    by="count", ascending=False
)
...
for loc in subcellular_location_counts["subcellular_location"]:
    if loc == "Other":
        continue
    sub_df = annotations_df[
        annotations_df["subcellular_location"].apply(lambda x: loc in x)
    ]
    sub_median_rmsf = []
    for p in sub_df["uniprot_id"]:
        sub_median_rmsf.append(np.median(planet_md_rmsf[p]))
