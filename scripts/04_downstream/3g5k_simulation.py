...
DCD_OUTPUT = (
    f"{config.PROCESSED_DATA_DIR}/3g5k_simulation/3g5k_sim_r{REPLICATE_SEED}.dcd"
)
PDB_OUTPUT = (
    f"{config.PROCESSED_DATA_DIR}/3g5k_simulation/3g5k_sim_r{REPLICATE_SEED}.pdb"
)
SCALAR_OUTPUT = (
    f"{config.PROCESSED_DATA_DIR}/3g5k_simulation/3g5k_scalars_r{REPLICATE_SEED}.csv"
)
...
integrator = LangevinIntegrator(TEMP, 1 / u.picosecond, TIME_STEP)
...
simulation.reporters.append(
    StateDataReporter(
        f"{LOG_OUTPUT}.progress",
        10 * reporter_interval,
        step=True,
        temperature=True,
        elapsedTime=True,
    )
